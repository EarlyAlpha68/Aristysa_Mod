...
public class CyberLegItem extends Item {
    protected int tier;
    protected String key = "cyberLegTier";
    public CyberLegItem(Settings settings, int tier){
        super(settings);
        this.tier = tier;
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        if (!world.isClient()) {
            if (!(((IEntityDataSaver) player).getPersistentData().getInt(key) == this.tier)) {
                ...
                player.sendMessage(Text.literal("You successfully implant yourself an Cyber Leg tier " + ((IEntityDataSaver) player).getPersistentData().getInt(key)));
                sync = true;
            } else {
    ...
    private static void alreadyHasIt(PlayerEntity player, int tiers){
        ItemStack oneItemStack = new ItemStack(ModItems.CYBERLEG_1, 1);
        ItemStack twoItemStack = new ItemStack(ModItems.CYBERLEG_2, 1);
        ItemStack threeItemStack = new ItemStack(ModItems.CYBERLEG_3, 1);
        if ((((IEntityDataSaver) player).getPersistentData().getInt("cyberLegTier") == 1 && (!(tiers == 1)))) {
            if (!player.getInventory().insertStack(oneItemStack)) {
                player.dropItem(oneItemStack, false);
            }
        }
        if ((((IEntityDataSaver) player).getPersistentData().getInt("cyberLegTier") == 2 && (!(tiers == 2)))) {
            if (!player.getInventory().insertStack(twoItemStack)) {
                player.dropItem(twoItemStack, false);
            }
        }
        if ((((IEntityDataSaver) player).getPersistentData().getInt("cyberLegTier") == 3 && (!(tiers == 3)))) {
